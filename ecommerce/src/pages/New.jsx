...
function New() {
    ...
    useEffect(() => {
        ...
        const fetchOffersProducts = async () => {
            ...
            setHeadsets(auriculares[auriculares.length - 1])
            const { data: teclados } = await axios.get("http://localhost:3000/api/teclados")
            setKeyboards(teclados[teclados.length - 1])
            const { data: monitores } = await axios.get("http://localhost:3000/api/monitores")
            setMonitors(monitores[monitores.length - 1])
            const { data: mouses } = await axios.get("http://localhost:3000/api/mouses")
            setMouses(mouses[mouses.length - 1])
        }
        fetchOffersProducts()

    }, [])

    const newProducts = [headsets, keyboards, monitors, mouses]
